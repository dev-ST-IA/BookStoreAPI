...
namespace BulkyBookBackEnd.Controllers
{
    ...
    public class StatsController : ControllerBase
    {
        ...
        //                        .Where(order =>
        //                            order.OrderUpdateDate >= dateRange.Start &&
        //                           order.OrderUpdateDate <= dateRange.End
        //                            );
        ...
        public async Task<IActionResult> GetTotalCustomers([FromQuery] DateRange dateRange)
        {
            try
            {
                var users = from customer in _context.Users
                            select customer;
                users = users.Where(user=>user.CreatedDateTime >=dateRange.Start &&
                                            user.CreatedDateTime <=dateRange.End
                                            );
        ...
        public async Task<IActionResult> GetSalesByCategory()
        {
            try
            {
                ...
                var sumOfSalesOfBooks = categories.Select(i => new {id=i.Id,category = i.Name ,sales=i.Books.Sum(x => x.Sales) });
                await sumOfSalesOfBooks.LoadAsync();


                if (sumOfSalesOfBooks!=null)
                {
                    return Ok(sumOfSalesOfBooks);
                }
                else
                {
                    return NotFound();
